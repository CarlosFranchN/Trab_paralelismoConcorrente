...
public class InsertionSort {
        ...
        for (int j = 0; j < 4; j++) {
            for (int i = 0; i < 5; i++) {
                ...
                if (j == 3) {
                    ...
                    insertionSortParalelo(array, numeroThreads);
    ...
    public static void salvarCSV(String metodo, int[][] tamanhosVetores, long[] tempos) {
        try (FileWriter writer = new FileWriter("insertionSort_" + metodo + "_resultados.csv")) {
    ...
    public static void insertionSortSerial(int[] arr) {
        int[] arrayOrdenadoSerial = Arrays.copyOf(arr, arr.length);
        insertionSort(arrayOrdenadoSerial);
    }

    public static void insertionSortParalelo(int[] arr, int numeroThreads) {
        int[][] subArrays = dividirArray(arr, numeroThreads);
        Thread[] threads = new Thread[numeroThreads];
        for (int i = 0; i < numeroThreads; i++) {
            final int[] subArray = subArrays[i];
            threads[i] = new Thread(() -> insertionSort(subArray));
            threads[i].start();
        }
        for (int i = 0; i < numeroThreads; i++) {
            try {
